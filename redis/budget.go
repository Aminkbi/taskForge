...
func (s *budgetStore) DependencyBudgetUsageSnapshots(ctx context.Context) ([]taskforge.DependencyBudgetUsageSnapshot, error) {
	...
	slices.Sort(names)
	snapshots := make([]taskforge.DependencyBudgetUsageSnapshot, 0, len(names))
	...
	for _, budget := range names {
		...
		snapshots = append(snapshots, taskforge.DependencyBudgetUsageSnapshot{
			Budget:   budget,
