...
func cloneFairnessPolicies(policies map[string]*FairnessPolicy) map[string]*FairnessPolicy {
	...
	cloned := make(map[string]*FairnessPolicy, len(policies))
...
func (b *Broker) fairnessPolicy(queue string) *FairnessPolicy {
...
func (b *Broker) queueStreamKey(queue, fairnessKey string) string {
	if b.fairnessPolicy(queue) == nil {
		return b.streamKey(queue)
	}
	return b.fairnessStreamKey(queue, NormalizeFairnessKey(fairnessKey))
}
...
func (b *Broker) reserveFair(ctx context.Context, queue, consumerID string) (taskforge.Delivery, error) {
	...
	for {
		...
		if err != nil {
			...
			return taskforge.Delivery{}, fmt.Errorf("reserve task: fairness wait: %w", err)
		}
		if len(values) == 0 {
			return taskforge.Delivery{}, taskforge.ErrNoTask
		}
...
func (b *Broker) reserveFairCandidate(ctx context.Context, queue, consumerName string, candidate fairnessKeySnapshot) (taskforge.Delivery, bool, error) {
	...
	if err != nil {
		...
		return taskforge.Delivery{}, false, fmt.Errorf("reserve task: fairness read: %w", err)
	}
	...
	if err != nil {
		return taskforge.Delivery{}, false, fmt.Errorf("reserve task: fairness decode: %w", err)
	}
	if msg.Queue == "" {
		msg.Queue = queue
	}
...
func (b *Broker) reclaimFairExpiredDelivery(ctx context.Context, queue, consumerName string) (taskforge.Delivery, bool, error) {
...
func (b *Broker) activeFairnessKeys(ctx context.Context, queue string) ([]string, error) {
...
func (b *Broker) FairnessMetricsSnapshot(ctx context.Context, queue string, now time.Time) ([]taskforge.FairnessMetricsSnapshot, error) {
	policy := b.fairnessPolicy(queue)
	if policy == nil {
		return nil, nil
	}

	buckets := make(map[string]taskforge.FairnessMetricsSnapshot)
	for _, bucket := range policy.Buckets() {
		rule, _ := policy.BucketRule(bucket)
		buckets[bucket] = taskforge.FairnessMetricsSnapshot{
			Bucket: bucket,
	...
	result := make([]taskforge.FairnessMetricsSnapshot, 0, len(buckets))
	for _, bucket := range policy.Buckets() {
