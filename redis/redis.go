...
func (b *Broker) ExtendLease(ctx context.Context, delivery taskforge.Delivery, ttl time.Duration) error {
	ctx, span := observability.StartQueueSpan(
		ctx,
		"taskforge.redis",
	...
	if err != nil {
		b.incrementLeaseExtensionFailure(taskforge.EffectiveQueue(delivery.Message))
	...
	for {
		...
		for _, pending := range pendingEntries {
			...
			if err != nil {
				...
				return taskforge.Delivery{}, false, fmt.Errorf("reclaim task: %w", err)
			}
			...
			if err != nil {
				return taskforge.Delivery{}, false, fmt.Errorf("reclaim task: claim expired delivery: %w", err)
			}
			if len(claimed) == 0 {
				cursor = pending.ID
				continue
...
func (b *Broker) pendingTask(ctx context.Context, streamKey, deliveryID string) (redis.XMessage, taskforge.Task, error) {
	messages, err := b.client.XRangeN(ctx, streamKey, deliveryID, deliveryID, 1).Result()
	if err != nil {
		return redis.XMessage{}, taskforge.Task{}, fmt.Errorf("load pending delivery %s: %w", deliveryID, err)
	}
	if len(messages) == 0 {
		return redis.XMessage{}, taskforge.Task{}, taskforge.ErrUnknownDelivery
	}

	msg, err := decodeTask(messages[0])
	if err != nil {
		return redis.XMessage{}, taskforge.Task{}, fmt.Errorf("decode pending delivery %s: %w", deliveryID, err)
	}
