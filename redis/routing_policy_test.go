...
func TestPolicyApplyRoutesFirstMatchingRule(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("ParseRoutingPolicyJSON() error = %v", err)
	}

	msg, placement := policy.Apply(taskforge.Task{
		ID:          "task-1",
		Name:        "billing.charge",
		Queue:       "ingress",
...
func TestPolicyPlaceFallsBackToDefaultPlacement(t *testing.T) {
	policy, err := ParseRoutingPolicyJSON([]byte(`{"default_queue":"bulk","default_shard":"shard-a"}`))
	if err != nil {
		t.Fatalf("ParseRoutingPolicyJSON() error = %v", err)
	}

	placement := policy.Place(taskforge.Task{ID: "task-1", Name: "unknown"})
...
func TestPolicyMatchesOriginalQueue(t *testing.T) {
	...
	placement := policy.Place(taskforge.Task{ID: "task-1", Queue: "critical.in"})
...
func TestParseJSONRejectsInvalidRules(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := ParseRoutingPolicyJSON([]byte(tt.raw)); err == nil {
				t.Fatal("ParseRoutingPolicyJSON() error = nil, want non-nil")
			}
		})
	}
}
