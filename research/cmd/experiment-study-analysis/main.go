...
func main() {
	root := flag.String("root", "second-wave", "study artifact directory")
	...
	for _, dir := range []string{resultsDir, figuresDir, paperDir} {
		if err := os.MkdirAll(dir, 0755); err != nil {
			expcli.Fatal("create %s: %v", dir, err)
		}
	}
	...
	if err != nil {
		expcli.Fatal("read paper template: %v", err)
	}
...
func mustWrite(path string, data []byte) {
	if err := os.WriteFile(path, data, 0644); err != nil {
		expcli.Fatal("write %s: %v", path, err)
	}
}
