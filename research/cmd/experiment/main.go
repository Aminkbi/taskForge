...
func main() {
	...
	db := flag.Int("redis-db", expcli.EnvInt("TASKFORGE_EXPERIMENT_REDIS_DB", 14), "dedicated Redis DB; must not contain production data")
	flag.Parse()
	if *scale < 1 || (*smoke && *scale != 1) {
		expcli.Fatal("scale must be >= 1 and smoke runs are always scale 1")
	}
	if *db < 1 {
		expcli.Fatal("refusing DB %d; choose a dedicated non-zero experiment DB", *db)
	}
	manifests, err := load(*workloads, *smoke, *scale)
	if err != nil {
		expcli.Fatal("load manifests: %v", err)
	}
	...
	if len(manifests) == 0 || len(variants) == 0 {
		expcli.Fatal("manifest or variant filter matched nothing")
	}
	...
	for _, manifest := range manifests {
		for _, variant := range variants {
			...
			if runErr != nil {
				expcli.Fatal("%s/%s: %v", manifest.Name, variant.Name, runErr)
			}
			redis := redisDelta(redisBefore, redisMetrics(ctx, client))
			samples := tracker.all()
			if len(samples) != manifest.Tasks {
				expcli.Fatal("%s/%s completed %d of %d tasks", manifest.Name, variant.Name, len(samples), manifest.Tasks)
			}
			...
			if err != nil {
				expcli.Fatal("write result: %v", err)
			}
			fmt.Println(path)
		}
	}
