...
func main() {
	input := flag.String("input", "data", "registered dataset directory (or raw result directory with -allow-partial)")
	results := flag.String("results", "results", "derived analysis output directory")
	figuresDir := flag.String("figures", "figures", "derived SVG figure directory")
	paperTemplate := flag.String("paper-template", "paper/paper.template.md", "paper template with generated-analysis tokens")
	paper := flag.String("paper", "paper/paper.md", "generated paper output")
	...
	if err != nil {
		expcli.Fatal("load experiment evidence: %v", err)
	}
	...
	for _, spec := range figures() {
		...
		if err := os.WriteFile(filepath.Join(*figuresDir, spec.file), svg, 0644); err != nil {
			expcli.Fatal("write %s: %v", spec.file, err)
		}
	}
