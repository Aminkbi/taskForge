...
func main() {
	...
	dataDir := flag.String("data", "second-wave/data", "staged study data directory")
	...
	if *binary == "" || *sourceCommit == "" {
		expcli.Fatal("-binary and -source-commit are required")
	}
...
func mustDigest(path string) string {
	file, err := os.Open(path)
	if err != nil {
		expcli.Fatal("open %s: %v", path, err)
	}
	defer file.Close()
	hash := sha256.New()
	if _, err := io.Copy(hash, file); err != nil {
		expcli.Fatal("digest %s: %v", path, err)
	}
