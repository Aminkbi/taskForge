...
func (w *Worker) run(ctx context.Context, drain <-chan struct{}, force <-chan struct{}, shutdownTimeout time.Duration) error {
	...
	w.publishAdaptiveState(ctx, state, taskforge.AdaptivePoolSnapshot{
		Pool:                  w.PoolName,
		Queue:                 w.Queue,
...
func (w *Worker) evaluateAdaptiveWindow(ctx context.Context, state *workerState) (taskforge.AdaptivePoolSnapshot, int, string, string, bool, error) {
	now := w.Clock.Now().UTC()
	queueSnapshot, err := w.QueueMetrics.QueueMetricsSnapshot(ctx, w.Queue)
	if err != nil {
		return taskforge.AdaptivePoolSnapshot{}, 0, "", "", false, fmt.Errorf("adaptive queue metrics %q: %w", w.Queue, err)
	}
	...
	snapshot := taskforge.AdaptivePoolSnapshot{
		Pool:                  w.PoolName,
		Queue:                 w.Queue,
		AdaptiveEnabled:       w.Adaptive.Enabled,
		ConfiguredConcurrency: float64(w.Concurrency),
...
func (w *Worker) processTask(ctx context.Context, delivery taskforge.Delivery, brokerLease *leaseHandle) error {
	...
	runningDelivery, err := transitionDelivery(delivery, taskforge.StateRunning)
	if err != nil {
		return fmt.Errorf("worker mark delivery running: %w", err)
	}
	if err := w.recordTaskState(execCtx, runningDelivery, taskforge.StateRunning, nil); err != nil {
		return err
	}
	...
	if err == nil {
		...
		succeededDelivery, transitionErr := transitionDelivery(runningDelivery, taskforge.StateSucceeded)
...
func (w *Worker) recordTaskState(ctx context.Context, delivery taskforge.Delivery, state taskforge.State, resultPayload []byte) error {
	if w.StateStore == nil {
		return nil
	}
...
func (w *Worker) tryAcquireBudget(ctx context.Context, delivery taskforge.Delivery) (*TaskBudget, bool, error) {
...
func (w *Worker) releaseBudget(ctx context.Context, budget *TaskBudget, delivery taskforge.Delivery) {
...
func (w *Worker) logLeaseLoss(delivery taskforge.Delivery, phase string, brokerLease *leaseHandle) {
	err := error(nil)
	if brokerLease != nil {
		err = brokerLease.Err()
	}
...
func (w *Worker) leaseOwnershipLost(err error) bool {
	return errors.Is(err, taskforge.ErrDeliveryExpired) || errors.Is(err, taskforge.ErrStaleDelivery)
}
...
func (w *Worker) publishAdaptiveState(ctx context.Context, _ *workerState, snapshot taskforge.AdaptivePoolSnapshot) {
	if w.Metrics != nil {
...
func (w *Worker) publishLifecycleSnapshot(ctx context.Context, snapshot taskforge.WorkerLifecycleSnapshot) {
...
func (w *Worker) lifecycleSnapshot(state *workerState) taskforge.WorkerLifecycleSnapshot {
	state.mu.Lock()
	defer state.mu.Unlock()
	return w.lifecycleSnapshotLocked(state)
}

func (w *Worker) LifecycleSnapshot() (taskforge.WorkerLifecycleSnapshot, bool) {
	if w == nil || w.runtimeState == nil {
		return taskforge.WorkerLifecycleSnapshot{}, false
	}
	return w.lifecycleSnapshot(w.runtimeState), true
}

func (w *Worker) lifecycleSnapshotLocked(state *workerState) taskforge.WorkerLifecycleSnapshot {
...
func transitionDelivery(delivery taskforge.Delivery, next taskforge.State) (taskforge.Delivery, error) {
	current := delivery.Execution.State
	if err := taskforge.ValidateTransition(current, next); err != nil {
		return taskforge.Delivery{}, err
	}
	delivery.Execution.State = next
	return delivery, nil
}
