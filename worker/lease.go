...
func startRenewalLoop(ctx context.Context, logger *slog.Logger, delivery taskforge.Delivery, every time.Duration, failureMessage string, renew func(context.Context) error) *leaseHandle {
	if every <= 0 {
		every = time.Second
	}
