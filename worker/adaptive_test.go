...
func TestAdaptiveConcurrencyScalesDownWithinBounds(t *testing.T) {
	...
	worker := &Worker{
		...
		QueueMetrics: staticQueueMetricsProvider{
			snapshot: taskforge.QueueMetricsSnapshot{Depth: 10, Reserved: 1},
		},
...
func TestAdaptiveConcurrencyCooldownBlocksRapidScaleUp(t *testing.T) {
	...
	worker := &Worker{
		...
		QueueMetrics: staticQueueMetricsProvider{
			snapshot: taskforge.QueueMetricsSnapshot{Depth: 5, Reserved: 0},
		},
		Clock: fixedClock{now: now},
	}
	state := &workerState{
...
func (s staticQueueMetricsProvider) QueueMetricsSnapshot(context.Context, string) (taskforge.QueueMetricsSnapshot, error) {
