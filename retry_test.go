...
func TestRetryPolicyJitterWithinBounds(t *testing.T) {
	...
	delay := policy.NextDelay(Task{ID: "task-1"}, 2)
	base := 2 * time.Second
...
func TestScheduleRetryPreservesOriginalTaskOwnership(t *testing.T) {
	t.Parallel()

	now := time.Date(2026, 7, 16, 10, 0, 0, 0, time.UTC)
	delivery := Delivery{
		Message:   Task{ID: "task-1", Headers: map[string]string{"original": "value"}},
		Execution: ExecutionMetadata{DeliveryCount: 1, FirstEnqueuedAt: now.Add(-time.Minute)},
	}
	next, ok, err := ScheduleRetry(delivery, string(FailureClassTransientRetryable), DefaultRetryPolicy(2), now)
	if err != nil || !ok {
		t.Fatalf("ScheduleRetry() = (%+v, %v, %v), want scheduled retry", next, ok, err)
	}
	if delivery.Message.Headers[HeaderRetryScheduledAt] != "" || delivery.Message.ETA != nil {
		t.Fatalf("ScheduleRetry() mutated owned delivery task: %+v", delivery.Message)
	}
	if next.Attempt != 1 || next.ETA == nil || next.Headers[HeaderRetryFailureClass] == "" {
		t.Fatalf("scheduled retry missing metadata: %+v", next)
	}
}
