...
func TestCompletionSemantics(t *testing.T) {
	t.Parallel()

	if CompletesDelivery(StateQueued) {
		t.Fatalf("CompletesDelivery(%q) = true, want false", StateQueued)
	}
	if !CompletesDelivery(StateSucceeded) {
		t.Fatalf("CompletesDelivery(%q) = false, want true", StateSucceeded)
	}
	if !CompletesDelivery(StateRetryScheduled) {
		t.Fatalf("CompletesDelivery(%q) = false, want true", StateRetryScheduled)
	}
	if CompletesTask(StateRetryScheduled) {
		t.Fatalf("CompletesTask(%q) = true, want false", StateRetryScheduled)
	}
	if !CompletesTask(StateDeadLettered) {
		t.Fatalf("CompletesTask(%q) = false, want true", StateDeadLettered)
	}
}
