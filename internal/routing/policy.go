...
func ParseJSON(data []byte) (*Policy, error) {
	...
	policy := &Policy{
		DefaultQueue: normalizeQueue(raw.DefaultQueue),
...
func (p *Policy) Place(msg broker.TaskMessage) Placement {
...
func (p *Policy) Apply(msg broker.TaskMessage) (broker.TaskMessage, Placement) {
...
func parseRule(index int, raw rawRule) (Rule, error) {
	...
	if name == "" {
		return Rule{}, fmt.Errorf("routing policy rule[%d] name is required", index)
	}
	...
	if rule.Match.empty() {
		return Rule{}, fmt.Errorf("routing policy rule %q must set at least one match condition", name)
	}
	return rule, nil
}

func parseDestination(index int, raw rawDestination) (Destination, error) {
	shardBy, err := normalizeShardBy(raw.ShardBy)
	if err != nil {
		return Destination{}, fmt.Errorf("routing policy rule[%d] destination.shard_by: %w", index, err)
	}
	destination := Destination{
	...
	if destination.Shard != "" && len(raw.Shards) > 0 {
		return Destination{}, fmt.Errorf("routing policy rule[%d] destination cannot set both shard and shards", index)
	}
	for _, shard := range raw.Shards {
		shard = strings.TrimSpace(shard)
		if shard == "" {
			return Destination{}, fmt.Errorf("routing policy rule[%d] destination shards must be non-empty", index)
		}
...
func (m Match) matches(msg broker.TaskMessage) bool {
...
func (d Destination) resolveShard(msg broker.TaskMessage, fallback string) string {
	if d.Shard != "" {
		return d.Shard
	}
	if len(d.Shards) == 0 {
...
func shardKey(msg broker.TaskMessage, shardBy string) string {
	switch {
...
func (p *Policy) defaultShard() string {
	if p == nil {
		return ""
	}
